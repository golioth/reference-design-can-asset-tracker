...
/* How long to wait between MPH display updates */
static int32_t _mph_update_delay_s = 1;
...
int32_t get_mph_update_delay_s(void)
{
	return _mph_update_delay_s;
}
		...
		const struct golioth_settings_value *value)
{
	...
	if (strcmp(key, "LOOP_DELAY_S") == 0) {
		...
		return GOLIOTH_SETTINGS_SUCCESS;
	} else if (strcmp(key, "MPH_UPDATE_DELAY_S") == 0) {
		/* This setting is expected to be numeric, return an error if it's not */
		if (value->type != GOLIOTH_SETTINGS_VALUE_TYPE_INT64) {
			return GOLIOTH_SETTINGS_VALUE_FORMAT_NOT_VALID;
		}
		...
		/* Only update if value has changed */
		if (_mph_update_delay_s == (int32_t)value->i64) {
			LOG_DBG("Received MPH_UPDATE_DELAY_S already matches local value.");
		} else {
			_mph_update_delay_s = (int32_t)value->i64;
			LOG_INF("Set MPH update delay to %d seconds", _mph_update_delay_s);
		}
		return GOLIOTH_SETTINGS_SUCCESS;
	}
